...
| Cart & checkout | `Cart`, `CartItem`, `Order` (now with `vatUgx`, `deliveryConfirmedAt`, `shippingZoneName` — added this session, see below), `OrderItem` (now with `vendorId`/`costUgxSnapshot`/`platformCommissionUgx`/`vendorPayoutUgx`/`vendorPayoutId`) |
| Shipping pricing | `ShippingZone` — added this session (per user decision, not in the original SRS): admin-managed towns-to-fee mapping, one fee per `DeliveryMethod`, replacing the old flat per-method fee table. `CheckoutService` matches the order's destination city against a zone's `towns` and charges that zone's fee for the chosen method, falling back to whichever zone has `isDefault: true` (seeded as "Rest of Uganda") when nothing matches. Seeded with 7 zones, priced by rough real-world distance from the Kampala warehouse (per explicit user correction — Central/Eastern/Western/Northern/Southern must each price differently, not share one flat "upcountry" rate): Kampala Metro, Central Uganda (Rest), Eastern, Southern, Western, Northern (farthest — no Same-day offered, `null`), and the "Rest of Uganda" fallback — see [`16-user-and-administrator-procedures.md`](./16-user-and-administrator-procedures.md) for how an admin edits zones/rates and [`23-data-dictionary.md`](./23-data-dictionary.md) for the full field list |
| Delivery (Driver App) | `Delivery` — 1:1 with `Order`, driver assignment + GPS pickup/delivery tracking (each snapshotted with a reverse-geocoded place name, added this session — see [`23-data-dictionary.md`](./23-data-dictionary.md)) + `priority` (commit `445a258`), not in the original SRS (per user request, see [`07-authentication-and-authorization.md`](./07-authentication-and-authorization.md) for the new `DRIVER` role and [`16-user-and-administrator-procedures.md`](./16-user-and-administrator-procedures.md) for how it's used). `User.driverStatus` (self-reported availability, commit `445a258`) lives on `User` itself, not this table |
