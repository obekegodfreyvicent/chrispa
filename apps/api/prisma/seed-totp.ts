...
// seed.ts sets twoFactorEnabled: true for these staff accounts (see the
// comment there) but never seeds an actual twoFactorSecret, so login
// deadlocks at the 2FA challenge with no way to produce a valid code. This
// mirrors TwoFactorService.enroll()'s own secret generation + encryption
// exactly (same otplib + totp-crypto.util functions) so the result is
...
// Render build-command step, then delete. (chris@chrispa.ug was already
// bootstrapped this same way in an earlier run.)
const prisma = new PrismaClient();

const EMAILS = ['patricia@chrispa.ug', 'grace@chrispa.ug'];
